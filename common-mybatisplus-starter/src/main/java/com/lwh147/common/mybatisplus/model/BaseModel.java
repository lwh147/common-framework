 ...
 * <p>
 * 但是其它数据表除了必须有上述字段外通常需要具备主键ID和逻辑删除字段，为此框架提供了 {@link AbstractDataModel}，数据表实体类可以选择
 * 继承此类
 * <p>
 * 如果想更进一步对数据表使用乐观锁，框架也提供了 {@link AbstractVersionModel}，对数据有并发控制要求的数据表实体类可以选择继承此类
 * <p>
 * 当然如果想完全自定义公共字段，那么可以直接继承 {@link Model} 类来编写自己的实体类基类
 *
