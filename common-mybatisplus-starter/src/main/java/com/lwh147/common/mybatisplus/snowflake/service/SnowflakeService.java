...
/**
 * 集群模式下雪花算法ID生成服务，各工作机器竞争抢占注册自己的工作机器ID和所属数据中心ID
 * 策略详情如下：
 * <p>
 * 1.应用启动后注入当前Bean时，在PostCounstruct阶段对雪花算法对象的工作机器ID和
 * 所属数据中心ID信息进行初始化
 * <p>
 * 2.首先去Redis中获取上一次被注册的节点 {@code worker} (相比从头开始效率高一些)
 * <p>
 * 3.调用 {@link Worker#next()} 方法获取下一个节点并尝试抢占（使用Redisson对当前
 * 节点加锁，加锁成功即视为抢占成功）
 * <p>
 * 4.如果抢占成功，使用抢占到的工作节点信息创建雪花算法对象并更新Redis上一次被注册的
 * 节点为当前节点；如果抢占失败，重复步骤3
 * <p>
...
public class SnowflakeService {
     ...
     * <p>
     * 没有在缓存中获取到时，说明当前没有实例抢占节点资源，返回一个初始Worker对象
     *
     * @return 获取到的Worker对象
     **/
